#include "Scripting\ScriptEngine.h"
#include "Scripting\ScriptManager.h"
#include "Input\InputHook.h"
#include "Hooking\Hooking.h"
#include "DirectX\D3d11Hook.h"
#include "Utility\Versioning.h"
#include "..\SDK\inc\enums.h"

using namespace Utility;

std::uint32_t g_ThreadHash = "main_persistent"_joaat;

BOOL APIENTRY DllMain( HINSTANCE hModule, DWORD dwReason, LPVOID /*lpvReserved*/ ) 
{
	switch ( dwReason ) 
	{
		case DLL_PROCESS_ATTACH: 
		{
			SetOurModuleHandle(hModule);
			
			// no launcher check
			if (auto p_launcherCheck = "E8 ? ? ? ? 84 C0 75 0C B2 01 B9 2F"_Scan) p_launcherCheck.nop(21);
			
			// no legals
			if (auto p_gameLegals = "72 1F E8 ? ? ? ? 8B 0D"_Scan) p_gameLegals.nop(2);

			static auto& versionTool = GTAVersion::GetInstance();
			g_GameVersion = versionTool.GameVersion();
			auto gta5directory = versionTool.GameDirectory();
			auto versionString = versionTool.VersionString();

			if (g_GameVersion == -1)
			{
				auto file = GetOurModuleFolder() + "\\Native_Registration.txt";
				if (DoesFileExist(file.c_str())) remove(file.c_str());
				LOG_FILE("Native_Registration", "%s", versionString.c_str());
				
				CreateElevatedThread([](LPVOID)->DWORD
				{ 
					Hooking::HookFunctions();
					while (true)
					{
						// Added safety pattern verification to prevent crashing on unmapped versions
						auto scanResult = "83 3D ? ? ? ? ? 8A D9 74 0A"_Scan;
						if (scanResult)
						{
							static eGameState* gameState = scanResult.add(2).rip(5).as<decltype(gameState)>();
							if (*gameState == GameStateMainMenu)
							{
								Hooking::UnHookFunctions();
								MessageBoxA(NULL, FMT("New Game Version %s", GTAVersion::GetInstance().VersionString().c_str()).c_str(), "ScriptHookV Incompatible", MB_OK | MB_TOPMOST);
								FreeLibraryAndExitThread(Utility::GetOurModuleHandle(), ERROR_SUCCESS);
								break;
							}
						}
						else
						{
							// Fallback failure exit if memory layout is completely unrecognized
							MessageBoxA(NULL, "Unknown game memory layout detected.", "ScriptHookV Error", MB_OK | MB_TOPMOST);
							FreeLibraryAndExitThread(Utility::GetOurModuleHandle(), ERROR_SUCCESS);
							break;
						}
						Sleep(100); // Prevent burning CPU cores in an infinite loop
					}
					return TRUE;
				});
			}
			else
			{
				g_IsRetail = !DoesFileExist((gta5directory + "\\steam_api64.dll").c_str());
				
				LOG_DEBUG("found GTA5 directory %s", gta5directory.c_str());
				LOG_DEBUG("detected GTA5 %s Version %s", g_IsRetail ? "Retail" : "Steam", versionString.c_str());

				// Adjusted minimum version boundary explicitly down to build 350
				if (g_GameVersion < VER_1_0_350_1_STEAM)
				{
					MessageBoxA(NULL, "Update to Version 1.0.350.1 or higher.", "Game Version Incompatible!",  MB_OK | MB_TOPMOST);
					FreeLibraryAndExitThread(hModule, 0);
					return TRUE;
				}

				// If it is retail, we only increment AFTER the minimum version check has passed securely
				if (g_IsRetail) g_GameVersion += 1;

				CreateElevatedThread([](LPVOID)->DWORD
				{
					if (!ScriptEngine::Initialize())
					{
						LOG_ERROR("Failed to initialize ScriptEngine");
						return FALSE;
					}	
					return TRUE;
				});
			}
			break;
		}
		case DLL_PROCESS_DETACH: 
		{
			g_HookState = HookStateExiting;
			break;
		}
	}

	return TRUE;
}
